...
interface HealthReport {
  ...
  services: {
    ...
    cep: ServiceHealth;
    sistema: { uptime?: number; environment?: string };
  };
}
...
export default function SystemStatusPage() {
  ...
  const [selectedService, setSelectedService] = useState<{ name: string, data: any } | null>(null);
  ...
  const renderStatusBadge = (service: ServiceHealth) => {
    ...
    return (
      <div className={`flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium border shadow-sm ${
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-8 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <button 
            onClick={() => router.back()} 
            className="text-sm text-gray-500 hover:text-gray-800 mb-2 flex items-center gap-2 transition-colors cursor-pointer"
          >
        ...
        <div className="flex items-center gap-3">
            ...
            <button
                onClick={checkHealth}
                disabled={loading}
                className={`px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-all flex items-center gap-2 cursor-pointer ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div 
            onClick={() => report && setSelectedService({ name: 'cep', data: report.services.cep })}
            className="bg-white dark:bg-zinc-800 p-6 rounded-xl border border-gray-200 dark:border-zinc-700 shadow-sm hover:shadow-md transition-all cursor-pointer hover:scale-[1.02] relative overflow-hidden group"
        >
            {/* CORREÇÃO: Ícone movido para baixo (-bottom-6) para não conflitar com o status */}
            <div className="absolute -bottom-6 -right-6 p-4 opacity-5 group-hover:opacity-10 transition-opacity z-0">
                <Icon icon={faMapMarkerAlt} className="w-32 h-32" />
            </div>
            
            {/* Conteúdo com z-10 para ficar acima do ícone de fundo */}
        ...
        <div 
            onClick={() => report && setSelectedService({ name: 'database', data: report.services.database })}
            className="bg-white dark:bg-zinc-800 p-6 rounded-xl border border-gray-200 dark:border-zinc-700 shadow-sm hover:shadow-md transition-all cursor-pointer hover:scale-[1.02] relative overflow-hidden group"
        >
             <div className="absolute -bottom-6 -right-6 p-4 opacity-5 group-hover:opacity-10 transition-opacity z-0">
                <Icon icon={faDatabase} className="w-32 h-32" />
            </div>
         ...
         <div 
             onClick={() => report && setSelectedService({ name: 'sistema', data: report.services.sistema })}
             className="bg-white dark:bg-zinc-800 p-6 rounded-xl border border-gray-200 dark:border-zinc-700 shadow-sm hover:shadow-md transition-all cursor-pointer hover:scale-[1.02] relative overflow-hidden group"
         >
             <div className="absolute -bottom-6 -right-6 p-4 opacity-5 group-hover:opacity-10 transition-opacity z-0">
                <Icon icon={faServer} className="w-32 h-32" />
            </div>
            ...
            <div className="relative z-10">
                <div className="flex justify-between items-start mb-4">
                    ...
                    {/* Padronizado para usar o mesmo badge dos outros */}
                    <div className="flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium border bg-green-100 text-green-700 border-green-200 shadow-sm">
